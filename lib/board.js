...
function Board() {
  ...
  this.defineStep("backlog", function (obj, cb) {
    // the backlog do nothing, it's there to enqueue tasks
    cb();
  });
}
  ...
  return function (callback) {
    ...
    s.func(obj, function (err) {
      ...
      if (next === undefined || next.current < next.opts.wip) {
        ...
        s.queue.push(newCallback);
      }

      var prev = steps[index - 1];
      if(prev !== undefined && prev.queue.length > 0) {
